...
class PolicyEngineTest(unittest.TestCase):
    ...
    def test_policy_isolation_faculty_then_regular(self):
        """Regression test for aliasing bug: faculty policy override should not
        affect subsequent regular member policy."""
        faculty_policy = self.engine.policy_for(Member("M-7", "Prof. Green", category="faculty"))
        regular_policy = self.engine.policy_for(Member("M-8", "Harry", category="regular"))

        # Faculty gets 42 days and no fines
        self.assertEqual(faculty_policy["loan_days"], 42)
        self.assertEqual(faculty_policy["daily_fine_cents"], 0)

        # Regular should get 21 days and standard fines, not corrupted by faculty policy
        self.assertEqual(regular_policy["loan_days"], 21)
        self.assertEqual(regular_policy["daily_fine_cents"], 25)
