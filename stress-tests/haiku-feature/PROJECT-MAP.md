...
plain-text summary report (totals, error rate, top endpoints, status breakdown, requests per hour).
...
- Report format: title, totals, error rate, top-N endpoints (`--top`, default 3), status
  breakdown by class (2xx/4xx/5xx), requests per hour (sorted chronologically).
...
- requests_per_hour values are time strings (e.g., '2026-07-01 10:00') that sort chronologically
  when sorted lexicographically; hour bucketing happens in timeutil.hour_bucket.
...
- 2026-07-16 Added "Requests per hour" section to text report (after "Status breakdown"),
  using existing requests_per_hour() from stats. JSON output unchanged. Hours are sorted
  chronologically by lexicographic sort (time strings are ISO-style yyyy-mm-dd hh:00).
