...
from .stats import error_rate, requests_per_hour, status_breakdown, top_endpoints, total_bytes
...
def render_report(entries, top=3):
    ...
    lines.append("")
    lines.append("Requests per hour:")
    for hour, count in sorted(requests_per_hour(entries).items()):
        lines.append("  %s  %d" % (hour, count))
    return "\n".join(lines) + "\n"
