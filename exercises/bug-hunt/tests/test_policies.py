...
class PolicyEngineIsolationTest(unittest.TestCase):
    """Resolving one category must never leak into later resolutions."""

    def test_faculty_resolution_does_not_pollute_regular(self):
        engine = PolicyEngine()
        engine.policy_for(Member("M-10", "Prof. Cho", category="faculty"))
        policy = engine.policy_for(Member("M-11", "Ada", category="regular"))
        self.assertEqual(policy["loan_days"], 21)
        self.assertEqual(policy["renewal_limit"], 2)
        self.assertEqual(policy["daily_fine_cents"], 25)
        self.assertEqual(policy["max_fine_cents"], 1000)

    def test_faculty_resolution_does_not_pollute_student_fines(self):
        engine = PolicyEngine()
        engine.policy_for(Member("M-10", "Prof. Cho", category="faculty"))
        policy = engine.policy_for(Member("M-12", "Ben", category="student"))
        self.assertEqual(policy["daily_fine_cents"], 25)
        self.assertEqual(policy["max_fine_cents"], 1000)

    def test_policy_for_does_not_mutate_engine_base(self):
        engine = PolicyEngine()
        before = dict(engine._base)
        engine.policy_for(Member("M-10", "Prof. Cho", category="faculty"))
        self.assertEqual(engine._base, before)
